...
export default function App() {
  ...
  // PWA Install State and instructions modal
  ...
  useEffect(() => {
    ...
    // If already running inside active PWA (standalone)
    if (window.matchMedia("(display-mode: standalone)").matches || (window.navigator as any).standalone) {
      setIsAppInstalled(true);
    }

    // Capture when application is successfully installed
    window.addEventListener("appinstalled", () => {
      setIsAppInstalled(true);
      setShowInstallBtn(false);
      setDeferredPrompt(null);
      showToastForDuration("PE Computers & BookShop POS successfully installed on device!", "success");
    });
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      if (outcome === "accepted") {
        setDeferredPrompt(null);
        setShowInstallBtn(false);
        showToastForDuration("App installation initiated!", "success");
      }
    } else {
      // Show customized modal if browser does not emit standard event (iOS Safari, Firefox, installed already)
      setShowPWAHelp(true);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans selection:bg-teal-500 selection:text-white">
      ...
      <header className="bg-[#0f172a] text-slate-100 border-b border-teal-950 px-6 py-5 shadow-lg relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center md:justify-between gap-4">
          ...
          <div className="flex items-center flex-wrap gap-3 text-xs font-semibold text-slate-400 font-mono justify-center md:justify-end">
            {isAppInstalled ? (
              <div className="flex items-center gap-1.5 bg-teal-950/40 text-teal-400 border border-teal-800/40 rounded-xl px-4 py-2.5">
                <Check className="w-3.5 h-3.5 text-teal-400" />
                <span>Installed on Mobile/PC</span>
              </div>
            ) : (
              <button
                onClick={handleInstallClick}
                className="flex items-center gap-1.5 bg-teal-600 hover:bg-teal-500 text-slate-900 font-bold px-4 py-2.5 rounded-xl transition-all cursor-pointer shadow-md shadow-teal-500/10 hover:shadow-teal-500/20 active:scale-95 duration-150"
              >
                <Download className="w-3.5 h-3.5 stroke-[2.5]" />
                <span>Install on PC / Android</span>
              </button>
            )}
      ...
      {/* PWA Multi-Platform Install Guide Modal */}
      {showPWAHelp && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200 text-slate-800">
            <div className="flex justify-between items-start mb-5">
              <div className="flex items-center gap-3">
                <div className="bg-teal-50 text-teal-600 p-2.5 rounded-2xl border border-teal-100">
                  <Download className="w-5 h-5 stroke-[2.5]" />
                </div>
                <div>
                  <h3 className="font-display font-bold text-base text-slate-900 leading-none">Install POS Application</h3>
                  <p className="text-[10px] text-slate-500 mt-1 font-semibold tracking-wide uppercase">Robust Offline System</p>
                </div>
              </div>
              <button
                onClick={() => setShowPWAHelp(false)}
                className="text-slate-400 hover:text-slate-600 transition p-1 text-sm bg-slate-100 hover:bg-slate-200/80 rounded-xl w-6 h-6 flex items-center justify-center cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-4 text-xs text-slate-600 font-sans">
              <div className="p-3.5 bg-teal-50/50 rounded-2xl border border-teal-100/50">
                <p className="font-bold text-teal-900 text-[10px] uppercase tracking-wider mb-1">⚡ Why Install?</p>
                <p className="text-[11px] text-teal-800 leading-relaxed font-medium">
                  Enjoy instantaneous startup and active checkout functionality even when completely offline. Your catalog and sales records stay safely synchronized in local storage.
                </p>
              </div>

              <div className="space-y-3">
                <div className="flex gap-2.5">
                  <div className="w-6 h-6 rounded-lg bg-slate-100 flex items-center justify-center font-bold text-slate-700 text-[10px] shrink-0">
                    <Monitor className="w-3.5 h-3.5 text-slate-500" />
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-800 text-[11px] uppercase tracking-wider">On Desktop / PC</h4>
                    <p className="text-[11px] text-slate-500 mt-0.5 leading-relaxed">
                      Click the installation arrow icon in your browser's address bar at the top-right, or choose <strong className="text-slate-800">Install PE Computers & Bookshop POS</strong> in Chrome or Edge menus.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-2.5">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 text-[11px] uppercase tracking-wider">On Android Devices</h4>
                    <p className="text-[11px] text-slate-500 mt-0.5 leading-relaxed">
                      Tap the <strong className="text-slate-800">⋮ (three dots)</strong> menu icon at the top-right of Chrome browser. Tap <strong className="text-slate-800">Install App</strong> or <strong className="text-slate-800">Add to Home Screen</strong>.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-2.5">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 text-[11px] uppercase tracking-wider">On Apple / iOS</h4>
                    <p className="text-[11px] text-slate-500 mt-0.5 leading-relaxed">
                      Open in standard Safari browser, tap the <strong className="text-slate-800">Share</strong> icon at the bottom, then choose <strong className="text-slate-800">Add to Home Screen</strong>.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100">
              <button
                onClick={() => setShowPWAHelp(false)}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-2.5 rounded-xl transition text-center text-xs font-display cursor-pointer"
              >
                Let me try
              </button>
            </div>
